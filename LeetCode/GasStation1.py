...
class Solution:
    ...
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        ...
        if i > len(gas): return -1 # No valid starting point found

        # Start journey
        tank = gas[i] - cost[i]
        starting_point = i
        # reached_start_point = False
        while True:  # tank >= cost[j] and j != starting_point:
            i += 1
            i = i % num_stops
            if tank < 0 or tank < cost[i]: return -1  # Not enough gas
            tank -= cost[i]
            if i == starting_point and tank >= 0: return starting_point
            tank += gas[i]
