...
or can be used to buy stuffs. Our hero Mr X, is in critical condition, has very less time left.
...
A person can pick any number of slips(even none, or all of them, or some of them) out of the N slips. But bank 
introduced a restriction, they announced one more number K. Restriction is that, if a person picks a slip A[i], 
then the next slip that he can choose to pick will be A[i+K+1]. It means there should be a difference of atleast K 
between the indices of slips picked.

Now slip(s) should be picked in such a way that their sum results in maximum positive time-amount sum possible with 
...
10
Explanation
1st Case: We can take slips { A[2]=2, A[6]=6, A[8]=2, A[10]=2 }, slips are atleast 1 indices apart this makes maximum 
sum, A[2]+A[6]+A[8]+A[10]=12
...
while t > 0:
    ...
    for i in range(1, k+1):
        dp.append(max([l2[i-1], dp[i-1]]))

    for i in range(k+1, n+1):
        dp.append(max([dp[i - 1], l2[i - 1] + dp[i - k - 1]]))
    print('dp is', dp, '-->', dp[n])
    t -= 1
